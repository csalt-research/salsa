...
def get_parser():
    ...
    parser.add_argument('--train-data-path', type=str, default="data/alpaca/train.pt", help='Path to the .pt file containing training data') 
    parser.add_argument('--val-data-path', type=str, default="data/alpaca/val.pt", help='Path to the .pt file containing validation data') 
...
def setup():
    ...
    languages = ["hi_in", "gu_in", "mr_in", "ta_in", "te_in", "ml_in", "pa_in"]

    print(f"Languages to be used: {languages}")

    # Load multilingual datasets
    train_datasets = []
    val_datasets = []
    for language in languages:
        train_data_path = args.train_data_path.replace("{LNH}", language)
        train_data = torch.load(train_data_path, map_location=torch.device('cpu'))
        train_datasets.append(train_data)

        val_data_path = args.val_data_path.replace("{LNH}", language)
        val_data = torch.load(val_data_path, map_location=torch.device('cpu'))
